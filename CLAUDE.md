...
```
src/main.ts                    → Adapter (Lifecycle, Poll-Zyklen je Konto, Objekt-IO)
src/lib/provider.ts            → UsageProvider-Vertrag + UsageSnapshot (transport-neutral, 5 Ordner)
src/lib/providers/<anbieter>.ts→ je Anbieter reines fetch+parse (kennt ioBroker NICHT)
src/lib/snapshot-tree.ts       → Snapshot → Objekt-Definitionen + Zustands-Werte (capability-driven)
src/lib/totals.ts              → total.* aus den Snapshots im Speicher
src/lib/pure-helpers.ts        → Konten-Tabelle parsen (API-Boundary), sanitizeId
...
1. **Admin-8-only** (krobi 2026-08-25): Zugänge über den zentralen Zugangsdaten-Speicher des Admin
   ...
   keine eigenen encryptedNative-Schlüsselfelder. `globalDependencies admin >= 8.4.15`.
